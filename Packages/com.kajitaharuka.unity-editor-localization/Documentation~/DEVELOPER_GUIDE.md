# UnityEditorLocalization 開発者ガイド

このガイドは、UnityEditorLocalizationを使って他のUnity Editor拡張を多言語化するための設計指針です。

実装前に動作イメージを確認したい場合は、Package Managerから`Localized Editor Window`サンプルを
importしてください。`Tools > UnityEditorLocalization > Samples > Localized Window`で、言語切り替え、
...
scope名やkeyのtypoを探すときは、`Preferences > UnityEditorLocalization`の「開発時診断」から未解決警告をONにできます。既定はOFFです。

ONの間は、`EditorL10n.Tr` / `TryTranslate`が未知scopeまたは未解決keyを検出したときに、同一scope/keyにつき一度だけConsoleへ警告します。`Tools > UnityEditorLocalization > Reload Catalogs`または`EditorL10n.Reload()`でカタログを再読み込みすると、警告済みの記録もリセットされます。
...
追加後は`Tools > UnityEditorLocalization > Validate Catalogs`を実行します。
...
- 全ロケールでdefaultLocaleと同値が正当な固定語（ファイル名・型名など）は、manifestの`fixedTerms`に宣言して未翻訳疑い警告の偽陽性を避けているか

## 任意依存（optional）として組み込む

販売パッケージなど、UnityEditorLocalizationを「入っていれば多言語化、無ければ単一言語」で組み込みたい場合は、ハード依存を避ける2アセンブリ方式を使います。本体assemblyは基盤を参照せず、ブリッジseam越しに文言取得とロケール追従を行い、基盤がある時だけコンパイルされる連携assemblyが多言語実装へ差し替えます。これにより、利用者が基盤を入れても入れなくてもコンパイル・表示が壊れません。
