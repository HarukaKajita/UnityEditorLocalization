...
- 同スキルのテンプレートファイル名をさらに短縮した（`IntegrationAsmdef.asmdef.txt` → `Asmdef.asmdef.txt`、`EditorL10nBridgeInstaller.cs.txt` → `BridgeInstaller.cs.txt`。`.meta` 同伴・GUID 維持）。本パッケージ自身のファイルパスが UAS 150字規則を超えていたため（出力ファイル名の規約は不変で、生成結果に影響はない）。
