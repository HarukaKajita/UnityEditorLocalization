...
- **スキル登録を実行しても内容が更新されないことがある不具合を修正した。** 登録先に実体ディレクトリ
  （symlink ではない本物のフォルダ）があると、中身を見ずに「成功」を返して何もしていなかった。
  スキルを更新したつもりで古い内容を使い続ける状態になり、登録状態のピルも「登録済み」と表示していた。
  実体ディレクトリのときは正本と中身を突き合わせ、違えば**その場で更新**する（余分なファイルは削除）。
  一致していれば何もしない。登録結果の要約も「リンク / 実体更新 / 変更なし / 失敗」の内訳を出す。
- **同梱スキルを増やしたときに、そのスキルが登録対象から漏れる問題を修正した。** 登録対象は固定配列に
  書かれていて、更新を忘れると新しいスキルが永久に登録されなかった。パッケージの `skills/` を
  実体から列挙するようにした（解決できないときだけ既知の一覧へ落とす）。
- **参照元が `Library/PackageCache` のときに警告を出すようにした。** git 依存で導入すると登録先は
  PackageCache を指すため、**リンク越しにスキルを編集してもパッケージの再解決で失われる**。
  スキル自体を直したい開発者が気づけるよう、登録結果に参照元パスと注意を明示する。
...
- **同梱スキル `editor-localization-translation-quality` に、屈折語で「訳さない語」を
  ...
  前置詞の直後にコードスパンを裸で置くと格が破綻する。カタログ上の実際の語形を示すための
  保護語は屈折させられないので、**格を担う普通名詞を前に置いて文を組み直す**のが正しい対処
  （`z \`zasób\`` → `z terminem \`zasób\``）。ウッチ大学ポーランド語相談室と
  ポーランド語編集者の言語相談の 2 出典で裏を取っている。機械で洗える形（前置詞 ×
  直後がコードスパン）も併記した。ソフトウェアの動作は変わらない。

## [1.3.4] - 2026-08-14

### Added
- **同梱スキル `editor-localization-translation-quality` に、Unity 公式の UI 用語との突き合わせを追加した。**
  - `references/unity-official-terms.json` … Unity 公式のエディタ翻訳から抽出した UI 用語 43 語 ×
    ja / ko / zh-Hans / zh-Hant の対照表。
  - `references/unity-official-terms.md` … 方針。**Unity のエディタ UI とドキュメントで訳語が割れている
    場合はエディタ UI を優先する**（拡張の文言は Unity のウィンドウの中に Unity 自身の文言と隣り合って
    出るため）。簡体字の「Asset」がエディタでは `资产`、ドキュメントでは `资源` と実際に割れている。
  - `scripts/check_unity_official_terms.py` … カタログを対照表と突き合わせる検査。
  あわせて SKILL.md へ「用語の是非を調査で決めるときの作法」を追加した（検索結果の AI 要約が
  原文を書き換えるため用語の綴りが争点の調査には使えないこと、機械翻訳ページを実例に数えないこと、
  取得できなかった出典を確認済みに混ぜないこと）。


### Fixed
- **日本語・中国語の UI 用語を Unity 公式のエディタ翻訳へ揃えた。表示される意味は変わらない。**
  日本語はアセットを指す `プロジェクト資産` を `プロジェクトのアセット` へ（1 箇所）、
  簡体字は `资源` → `资产`（3 箇所）、繁体字は `資產` → `資源`（3 箇所）。
  Unity のエディタ翻訳とドキュメントで訳語が割れている語は、**開発者が実際に目にする
  エディタ UI の訳語を優先する**方針にした。
...
### Changed
- **動作確認済みの環境に Windows 11 を追加した。** これまで macOS でしか実機確認していなかったが、
  Windows 11 / Unity 2022.3.22f1 で、19 ロケールの切替と表示が実際に動作すること、
  自動テスト全 44 件が通ることを実機で確認した。**Linux は実機での確認をしていない**（従来どおり）。
  ソフトウェアの動作は変わらない。
