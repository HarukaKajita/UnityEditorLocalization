...
`com.kajitaharuka.editor-localization`（表示名: UnityEditorLocalization）です。実体はすべて
[Packages/com.kajitaharuka.editor-localization/](Packages/com.kajitaharuka.editor-localization/) 配下にあります。
...
`Packages/com.kajitaharuka.editor-localization/` 内で行います。

- Unity バージョン: 2022.3（`Packages/com.kajitaharuka.editor-localization/package.json` の `unity` を参照）
...
`Packages/com.kajitaharuka.editor-localization/Tests/Editor/` には EditMode テスト用の
...
`testables` には `com.kajitaharuka.editor-localization` を含めます。

## アーキテクチャ

文言は「scope（名前空間）× ロケールタグ × key」で引きます。中核の流れは次の通りです。

1. **カタログ探索とロード** — [Editor/Core/EditorL10nCatalog.cs](Packages/com.kajitaharuka.editor-localization/Editor/Core/EditorL10nCatalog.cs)
...
2. **公開 API とロケール解決** — [Editor/Core/EditorL10n.cs](Packages/com.kajitaharuka.editor-localization/Editor/Core/EditorL10n.cs)
...
3. **表示言語の永続化** — [Editor/Core/EditorL10nPreferences.cs](Packages/com.kajitaharuka.editor-localization/Editor/Core/EditorL10nPreferences.cs)
...
4. **UI Toolkit バインド層** — [Editor/UI/EditorL10nUi.cs](Packages/com.kajitaharuka.editor-localization/Editor/UI/EditorL10nUi.cs)
...
5. **検証** — [Editor/Validation/EditorL10nValidator.cs](Packages/com.kajitaharuka.editor-localization/Editor/Validation/EditorL10nValidator.cs)、
   **Preferences UI** — [Editor/Settings/EditorL10nSettingsProvider.cs](Packages/com.kajitaharuka.editor-localization/Editor/Settings/EditorL10nSettingsProvider.cs)、
   **JSON モデル** — [Editor/Core/EditorL10nJson.cs](Packages/com.kajitaharuka.editor-localization/Editor/Core/EditorL10nJson.cs)（`JsonUtility` 用の `[Serializable]` DTO 群）。
...
- key は機能領域から始め役割が分かる名前にし、**表示文言や表示順を key に含めない**（[DEVELOPER_GUIDE.md](Packages/com.kajitaharuka.editor-localization/Documentation~/DEVELOPER_GUIDE.md) の命名規約に従う）。
...
- [Packages/com.kajitaharuka.editor-localization/Documentation~/OPTIONAL_INTEGRATION.md](Packages/com.kajitaharuka.editor-localization/Documentation~/OPTIONAL_INTEGRATION.md): UnityEditorLocalization を **任意依存（optional）** として組み込むための 2 アセンブリ方式・命名規約・version define の注意・チェックリスト（リファレンス実装: ExportPackageExtension）
