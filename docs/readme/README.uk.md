...
Легка основа локалізації, що працює **лише в редакторі (Editor-only)**, для розширень редактора Unity. Текст інтерфейсу (підписи Inspector, HelpBox, кнопки, логи Console, індикатори прогресу) береться з каталогів перекладу за scope, індексованих за **scope × locale × key**. Щоб додати мову, достатньо додати файл JSON із перекладами та зареєструвати його в manifest'і — без змін у C#.
...
- **19 мов** постачаються в UI налаштувань (у вбудованому прикладі каталогу — `ja` та `en`).
...
Закріпіть версію, додавши тег релізу, наприклад `#<tag>`; замість `<tag>` підставте ім'я тега з Releases репозиторію. Той самий URL можна додати безпосередньо в `Packages/manifest.json`, у розділ `dependencies`.
...
`.unitypackage` не містить ні `Samples~/`, ні `Documentation~/`, оскільки AssetDatabase Unity не обробляє теки з `~`. Якщо потрібні приклад і докладні посібники, скористайтеся `.tgz`, git URL або VPM.
