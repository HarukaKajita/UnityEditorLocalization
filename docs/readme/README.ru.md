...
Лёгкая **работающая только в редакторе (Editor-only)** основа локализации для расширений редактора Unity. Текст интерфейса (подписи Inspector, HelpBox, кнопки, логи Console, индикаторы прогресса) берётся из каталогов перевода по scope, индексируемых по **scope × locale × key**. Чтобы добавить язык, достаточно добавить JSON-файл с переводами и зарегистрировать его в манифесте — без изменений в C#.
...
- **19 языков** поставляются в UI настроек (во встроенном примере каталога — `ja` и `en`).
...
Закрепите версию, добавив тег релиза, например `#<tag>`; вместо `<tag>` подставьте имя тега из Releases репозитория. Тот же URL можно добавить напрямую в `Packages/manifest.json`, в раздел `dependencies`.
...
`.unitypackage` не содержит ни `Samples~/`, ни `Documentation~/`, поскольку AssetDatabase Unity не обрабатывает папки с `~`. Если нужны пример и подробные руководства, используйте `.tgz`, git URL или VPM.
