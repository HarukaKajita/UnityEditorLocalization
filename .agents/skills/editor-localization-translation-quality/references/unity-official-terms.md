...
### 家の用語より画像の鮮度を採ることがある（2026-08-26 の判断）

UEWCE の `既定ターゲット` は家の用語（`対象`）から外れているが、**直さないと決めた。**

- **公式訳は `ターゲット`** なので、このラベルは誤りではない。外れているのは家の用語の側である。
- このラベルは**商品ページと BOOTH ギャラリーの画像 2 枚に写っている**
  （`assets/usage-project-settings.png` / `publish-assets/booth-06-settings.png`）。
- 画像は **macOS の Retina 2x**（撮影メタデータに `pixelsPerPoint: 2.0` / 2000x1364）で撮られており、
  Windows 機は `pixelsPerPoint = 1`（実測）。**UEWCE は実機の ppp より大きい値への拡大を拒否する**仕様
  なので、同じ窓を撮っても 1000x682 にしかならず、同等の差し替えができない。
  解決先パス・プレビューのファイル名・フォントのラスタライズも原本と変わる。
- **文言を 1 語揃える利得より、販売面の画像が実物と食い違う害の方が大きい**と判断した。
  `familyTerms` の `exceptWithin` に入れてあるので検査は鳴らず、判断の理由はここに残る。

**一般化すると: 家の用語の適用は、その語が販売面の画像に焼き込まれているかを見てから決める。**
画像は grep で見つけられない唯一のサーフェスなので、文言だけを見て直すと販売面が静かに古くなる。
